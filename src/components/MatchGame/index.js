...
class MatchGame extends Component {
  state = {
    ...
    isGameOver: false,
    setDisplayedImageId: imagesList[0].id,
    index: 0,
  }
  ...
  thumbnaileClicked = thumbnailId => {
    const {setDisplayedImageId} = this.state
    if (thumbnailId === setDisplayedImageId) {
      this.randomIndex()
  ...
  renderThumbnailImage = () => {
    ...
    return (
      <ul className="thumbnail-container">
        {filterImagesList.map(eachThumbnail => (
          <ThumbnailItem
            thumbnailDetails={eachThumbnail}
            key={eachThumbnail.id}
            thumbnaileClicked={this.thumbnaileClicked}
          />
        ))}
      </ul>
    )
  ...
  randomIndex = () => {
    const lengthImgList = imagesList.length
    const index = Math.floor(Math.random() * lengthImgList)
    const newId = imagesList[index].id
    this.setState({index, setDisplayedImageId: newId})
  }

  renderDisplayImage = () => {
    const {index} = this.state
    const randomItem = imagesList[index]
  ...
  playAgainButton = () => {
    this.setState({score: 0, timer: 60, isGameOver: false}, this.getTimer)
  }
  ...
  render() {
    ...
    return (
      <div className="app-container">
        ...
        {isGameOver ? (
          <ScoreCard score={score} playAgainButton={this.playAgainButton} />
        ) : (
          this.renderMainCard()
        )}
      </div>
